# Step 10: Networking from the CLI - curl, openssl, dig, tcpdump, iptables
...
sudo sh -c 'printf "127.0.0.1 mytest.example.com\n" >> /etc/hosts'

# Verify it works
curl http://mytest.example.com:8080/
# This connects to 127.0.0.1 regardless of what DNS says

# Remove the override when done
sudo sed -i '/mytest.example.com/d' /etc/hosts
```
...
**Step 11: Process Supervision - systemd, supervisord, and keeping things running.** You
