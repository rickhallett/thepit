...
**Expected output format:** Commands and their output, with a brief explanation of what
each field means.

**What you are learning:** The kernel exposes all process metadata via `/proc`. This is
your primary diagnostic tool for understanding what any process on the system is doing.
...
1. Creates a pipe with `os.pipe()`
2. Forks with `os.fork()`
3. In the child: closes the read end, writes "Hello from child process (PID: {pid})" to
   the write end, closes the write end, exits
4. In the parent: closes the write end, reads from the read end until EOF, prints what it
   received, closes the read end, calls `os.wait()` to reap the child

This replicates what the shell does for `echo "Hello from child" | cat`, but without the
shell's machinery. You are doing the fork, the pipe, and the fd management yourself.

**Stretch goal:** Modify the script to create a two-stage pipeline (three processes, two
pipes), equivalent to `echo "hello" | tr a-z A-Z | cat`. The parent creates both pipes
and forks two children, wiring each one's stdin/stdout to the appropriate pipe ends using
`os.dup2()`.

<details>
<summary>Solution for the basic version</summary>
...
def main():
    ...
    if pid == 0:
        # Child process
        os.close(read_fd)  # Child doesn't read from the pipe
        message = f"Hello from child process (PID: {os.getpid()})\n"
        os.write(write_fd, message.encode())
        os.close(write_fd)
        os._exit(0)  # Use _exit in child after fork, not sys.exit
    else:
        # Parent process
        os.close(write_fd)  # Parent doesn't write to the pipe

        # Read until EOF
        data = b""
        while True:
            chunk = os.read(read_fd, 4096)
            if not chunk:  # EOF - write end is closed
                break
            data += chunk
...
Write a bash script (`/tmp/signal_lab.sh`) that:

1. Creates a temporary directory with `mktemp -d`
2. Creates three temp files inside it
3. Installs a `trap` handler for SIGTERM, SIGINT, and EXIT that:
   - Prints "Received signal, cleaning up..."
   - Removes the temp directory and all contents
   - Prints "Cleanup complete"
4. Sleeps for 60 seconds (simulating work)

Then, in a separate terminal (or using background execution and `kill`):

a. Run the script, send it SIGTERM, and verify cleanup happened
b. Run the script, send it SIGKILL, and verify cleanup did NOT happen (temp dir leaked)
c. Run the script, press Ctrl-C (SIGINT), and verify cleanup happened

**Record the exit code after each signal.** Explain why SIGTERM gives a different exit
code than SIGKILL.
...
cleanup() {
  ...
  rm -rf "$TMPDIR"
  printf 'Cleanup complete. Temp dir removed: %s\n' "$TMPDIR"
}
...
```bash
# Test SIGTERM
bash /tmp/signal_lab.sh &
PID=$!; sleep 1; kill -TERM $PID; wait $PID 2>/dev/null; printf 'Exit: %s\n' "$?"
# Expected: cleanup runs, exit code 143 (128+15)

# Test SIGKILL
bash /tmp/signal_lab.sh &
PID=$!; sleep 1; kill -KILL $PID; wait $PID 2>/dev/null; printf 'Exit: %s\n' "$?"
# Expected: NO cleanup, exit code 137 (128+9), temp dir still exists

# Test SIGINT (Ctrl-C sends this)
bash /tmp/signal_lab.sh &
PID=$!; sleep 1; kill -INT $PID; wait $PID 2>/dev/null; printf 'Exit: %s\n' "$?"
# Expected: cleanup runs, exit code 130 (128+2)
```
...
Demonstrate three pipeline failure scenarios and show how to detect each one:

**Scenario A: Silent failure**

```bash
# This pipeline "succeeds" but produces wrong results
curl -s https://httpbin.org/status/404 | wc -c
printf 'Exit code: %s\n' "$?"
# Question: What exit code do you get? What did wc count?
```

**Scenario B: set -e does not help**
...
# Does this script exit here? Why or why not?
false | true
...
Run it and explain the output.

**Scenario C: pipefail to the rescue**
...
**Scenario D: PIPESTATUS forensics**

Write a three-stage pipeline where the middle command fails. Use `${PIPESTATUS[@]}` to
identify which stage failed:

```bash
printf 'line1\nline2\nline3\n' | grep -c nonexistent | cat
printf 'PIPESTATUS: %s %s %s\n' "${PIPESTATUS[0]}" "${PIPESTATUS[1]}" "${PIPESTATUS[2]}"
```

Explain what each PIPESTATUS value means.

> **AGENTIC GROUNDING:** When you review an agent-generated script, check for these
> patterns. The difference between a script that silently swallows errors and one that
> catches them is often just the addition of `pipefail` and proper `PIPESTATUS` checking.
> Agents do not add these unless prompted. Build the habit of adding them yourself.
...
**Part A: Create a zombie**

```python
#!/usr/bin/env python3
"""Create a zombie process and observe it."""
import os
import time
import subprocess
...
if pid == 0:
    ...
    print(f"Parent PID: {os.getpid()}")
    print(f"Child PID: {pid}")
    print(f"Child has exited but parent hasn't called wait().")
    print(f"Run in another terminal: ps -o pid,ppid,state,comm -p {pid}")
    time.sleep(30)
    print("Parent exiting without reaping child.")
```

Run this and in another terminal, verify the zombie exists:
...
**Part B: Reap the zombie**

Modify the script to reap the zombie after 10 seconds:

```python
    time.sleep(10)
    child_pid, status = os.waitpid(pid, 0)
    print(f"Reaped child {child_pid}, status: {os.waitstatus_to_exitcode(status)}")
    time.sleep(20)  # Now observe: zombie is gone
```

**Part C: Connect to agents**

Think about this scenario: an agent framework spawns subprocess after subprocess (shell
commands, linters, test runners) but uses `subprocess.Popen()` without calling `.wait()`
or `.communicate()`. Each subprocess becomes a zombie after it finishes. Over a long
agent session, hundreds of zombies accumulate.

Run `ps aux | grep -c ' Z '` on a system where an agent has been running for a while.
If the count is non-zero, you have found zombie leaks.

> **AGENTIC GROUNDING:** Agent frameworks like LangChain and AutoGPT spawn subprocesses
> to execute code. If the framework's subprocess management has bugs (and it has - this
> is an observed, not hypothetical, failure mode), zombies accumulate. The fix is either
> in the framework code (call `.wait()`) or at the system level (the parent process must
> handle SIGCHLD). You need to understand zombies to diagnose this.
...
Start a Python HTTP server and investigate its file descriptors:
...
# 2. Use lsof to get detailed info
lsof -p $SERVER_PID

# 3. Find the listening socket
lsof -p $SERVER_PID -i

# 4. Now make a connection (in another terminal or background):
curl -s http://localhost:9876/ > /dev/null &
sleep 0.5

# 5. Check fds again - what changed?
ls -la /proc/$SERVER_PID/fd/
lsof -p $SERVER_PID -i

# Questions:
# - Which fd is the listening socket?
# - Which fd is the accepted connection?
# - What other fds does the process have open, and why?

# Clean up
kill $SERVER_PID
```

**Stretch goal:** Write a bash function `fd_report()` that takes a PID and produces a
human-readable summary of all its file descriptors, categorised by type (file, pipe,
socket, device, other):
...
fd_report() {
  ...
  printf 'File descriptor report for PID %s (%s)\n' \
    "$pid" "$(cat /proc/$pid/comm)"
  printf '=%.0s' {1..50}; printf '\n'

  for fd in /proc/$pid/fd/*; do
    local fd_num=$(basename "$fd")
    local target=$(readlink "$fd" 2>/dev/null || printf 'unreadable')
    local fd_type="unknown"

    case "$target" in
      pipe:*)     fd_type="pipe" ;;
      socket:*)   fd_type="socket" ;;
      /dev/*)     fd_type="device" ;;
      /proc/*)    fd_type="proc" ;;
      anon_inode:*) fd_type="anon_inode" ;;
      /*)         fd_type="file" ;;
    esac
...
2. Why does Unix split process creation into fork() and exec()? What does this enable
   that a monolithic CreateProcess cannot?

3. What is a file descriptor? What is the relationship between fd 1 in a process and the
   terminal, a file, or a pipe?

4. When the shell executes `cmd1 | cmd2`, what sequence of syscalls occurs? Why is it
   critical to close unused pipe ends?

5. What is the difference between SIGTERM and SIGKILL? Why does SIGKILL exist?

6. What is a zombie process? Why does it exist? How do you prevent them?

7. Why does `set -e` not catch pipeline failures? What do you add to fix this?

8. What exit code does a process killed by SIGKILL have? How do you calculate this from
   the signal number?

9. Why does `echo "value" | some_tool` produce different results than
   `printf 'value' | some_tool`?

10. If `curl` fails in `curl url | jq '.field'`, what does `$?` report? Why?

If you can answer all ten from memory, you have the foundation for everything that
follows.
...
- **Advanced Programming in the UNIX Environment** - W. Richard Stevens, Stephen Rago
  ...
  The definitive reference for Unix systems programming.

- **The Unix Programming Environment** - Kernighan and Pike (1984). Chapter 7 covers
  the process model from the user's perspective. Dated but foundational.

- **`man 2 fork`**, **`man 2 exec`**, **`man 2 pipe`**, **`man 7 signal`** - the actual
  specifications. Read the man pages. They are precise.

- **McIlroy's pipe memo** - "Summary -- what's most important. To put my strongest
  concerns into a nutshell: 1. We should have some ways of coupling programs like garden
  hose -- screw in another segment when it becomes when it becomes necessary to massage
  data in another way." (1964). The original document that led to pipes in Unix.
