...
### 4.6 Lexicon terms without engineering grounding

Every Lexicon entry must pass two gates before admission:

1. **Synonym gate.** The term must have a direct synonym in an established software engineering rubric (IEEE, SWEBOK, DORA metrics, Leveson STAMP, NASA flight rules, whatever fits). If no synonym exists, the term is either genuinely novel (rare) or self-referential jargon (common). "Category One" must map to "flawed mental model" (Leveson) or "Byzantine fault" or something that existed before we named it. If it only exists inside this system, it is not a principle -- it is a local habit.

2. **Cross-model adversarial gate.** The definition must survive cross-examination by a different model family (not Claude). Submit the term, definition, and the claimed engineering synonym to GPT-4o or Gemini. Ask: "Is this a real engineering concept or project-specific jargon?" If a different architecture, trained on different data, with no exposure to this project's context, cannot recognise the engineering substance, the term fails admission. If it passes, the cross-model verdict is recorded alongside the entry.

**Phase 2 rule:** No term enters the Lexicon without an engineering synonym and a cross-model smoke test. The Lexicon is grounded in the field or it is grounded in nothing.
