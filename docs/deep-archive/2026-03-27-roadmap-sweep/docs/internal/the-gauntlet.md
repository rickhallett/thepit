...
**Invocation:**
```bash
just darkcat          # Claude (DC-1)
just darkcat-openai   # Codex (DC-2)
just darkcat-gemini   # Gemini (DC-3)
# or
just darkcat-all      # all three in parallel
```
...
**Invocation:**
```bash
uv run pitkeel-py/pitkeel.py                                # review signals
uv run pitkeel-py/pitkeel.py state-update --officer <name>  # update state
```
...
| Data | Location | Format | Retention |
|------|----------|--------|-----------|
| Checklist marks | `plans/REVIEW-CHECKLIST.md` (planned, not yet created) | Markdown checkboxes | Updated in place |
...
└── weaver/
    ...
    └── commendations.log       # Commendations log

scripts/
├── darkcat.md                  # The adversarial review prompt (portable)
└── prepare-commit-msg          # Git hook (pitkeel signals + trailers)

pitkeel/                            # Go CLI (legacy)
pitkeel-py/                         # Python CLI (active: pitkeel.py, daemon.py, analysis.py)

.keel-state                     # Operational state (runtime-generated by pitkeel, gitignored)
```
...
just darkcat              # DC-1 (Claude)
just darkcat-openai       # DC-2 (OpenAI)
just darkcat-gemini       # DC-3 (Gemini)
just darkcat-synth        # DC-SYNTH (convergence)
uv run pitkeel-py/pitkeel.py        # review signals

# Or the full gauntlet (automated)
just gauntlet             # runs all darkcats + synth + pitkeel
```
