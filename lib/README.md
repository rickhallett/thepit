...
72 shared utility modules organized in a flat directory. Types are co-located with their modules rather than in a separate types file. The modules cluster into 11 functional domains.
...
- **Flat directory** — 72 files in one folder. The naming convention creates implicit groupings (`agent-*`, `credit-*`, `response-*`, `research-*`). At the current scale this is approaching the threshold where a subdirectory split by domain would improve discoverability.
