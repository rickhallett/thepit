...
export function useBoutSharing({
  ...
  const copyTranscript = async () => {
    ...
    setCopied(true);
    trackEvent('bout_shared', { bout_id: boutId, preset_id: preset.id, method: 'copy_header' });
  ...
  const copyMessageShare = async (payload: string, messageId: string) => {
    ...
    trackEvent('bout_shared', { bout_id: boutId, preset_id: preset.id, method: 'copy_message' });
