...
```signal
S1_coherence := PR.describable_in_1_sentence & PR.verifiable_in_isolation
PR1 := pure_types + validation + regex | no_LLM | no_DB_writes | no_external_deps
PR2 := LLM_integration + streaming + route | depends(PR1) | requires(ai, @ai-sdk/anthropic)
RULE := PR1.merge -> verify -> PR2.merge -> verify | sequential [stacked_prs]
```
...
plan_13 := wires(credits, persistence) into existing engine LATER
bout_engine := works_without_credits | feature_flag_guards_all_credit_paths
RATIONALE := credits.domain == 4_files + 3_plans | doubles_PR_scope_if_included
```
...
SEQUENCE := PR1 -> gate -> review(@Weaver) -> merge -> PR2 -> gate -> review(@Weaver) -> merge
POST_MERGE := verify(gate.on_main) -> stain(diff, watchdog_taxonomy) -> advance
```
