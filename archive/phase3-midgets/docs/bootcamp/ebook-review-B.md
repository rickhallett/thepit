# Technical Book Chapter Review - Structural Dependency Assessment

You are a curriculum designer reviewing a chapter index for a technical book.
The book has 51 chapters across 5 parts. The author needs to cut significantly.

For each chapter, assess what happens to the rest of the book if it is removed.
Consider both the formal dependency graph (listed per chapter) AND conceptual
dependencies where later chapters assume knowledge from earlier ones without
the formal link.

Rate each chapter:

- **LOAD-BEARING** - Removing it breaks 3+ other chapters or removes a concept that later chapters assume without re-explaining
- **SUPPORTING** - 1-2 chapters depend on it, but a brief recap or prerequisite note could substitute
- **LEAF** - No chapter depends on it; removal has zero impact on other chapters
...
structural_necessity:
  I.1: { rating: "LOAD-BEARING|SUPPORTING|LEAF", reason: "<1 sentence>" }
  I.2: { rating: "...", reason: "..." }
  # all 51 chapters
```
